...
def load():
  ...
  for rec in csv.reader(open(os.path.join(os.curdir, 'su.csv'), 'rb'), delimiter=';'):
...
if __name__ == '__main__':
  ...
  for s in telephoneDir.subdivision.iterSubdivision():
      ...
      for r in telephoneDir:
        #if r.collaborator in s and r.collaborator.family.find('') >= 0:
          print r.telephone.number, "%s %s. %s. %s."% \
                (r.collaborator.family, r.collaborator.name[:2], r.collaborator.patronym[:2], find(r.collaborator, telephoneDir.subdivision))
	  #print('###');
      break
...
'''  
    for s in telephoneDir.subdivision.iterSubdivision():
	    for r in telephoneDir:
		    if r.collaborator in s and r.collaborator.family.find('а') >= 0 or r.collaborator.family.find('ф') >= 0:
			    print r.telephone.number, "%s %s. %s."% \
                    (r.collaborator.family, r.collaborator.name[:2], r.collaborator.patronym[:2])
	    break
'''
